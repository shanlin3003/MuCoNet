...
class PatchExpand2D(nn.Module):
    def __init__(self, dim):
        ...
        self.dim = dim
        self.expand = nn.Conv2d(self.dim, self.dim//2, kernel_size=1)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if upsample is not None:
            self.upsample = upsample(self.dim)
        else:
            self.upsample = None
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 9, 2], depths_decoder=[2, 2, 2, 1],
                 dims=[96, 192, 384, 768], dims_decoder=[768, 384, 192, 96], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False, **kwargs):
        super().__init__()
        self.num_layers = len(depths)
        self.decoder_num_layers = len(depths_decoder)
        ...
        # for i_layer in range(self.decoder_num_layers):
        #     layer = VSSLayer_up(
        ...
        #         norm_layer=norm_layer,
        #         upsample=PatchExpand2D if (i_layer < self.decoder_num_layers - 1) else None,
        #         use_checkpoint=use_checkpoint,
        #     )
    ...
    def forward_features(self, x):
        ...
        outs = [v.permute(0,3,1,2) for v in skip_list]
        return outs, skip_list
    ...
    def forward_features_up(self, x, skip_list):
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x, y = layer_up(x+skip_list[-inx])
            outs.append(y.permute(0,3,1,2))
        return outs